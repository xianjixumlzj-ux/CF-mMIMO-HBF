...
for bs_id in active_bs_ids:
    ...
    # 重塑为（样本数, 用户数, 天线数）→ 注意这里先用户数后天线数（适配后续维度）
    target_samples = min(sample_num, len(cir_complex))
    ue_per_ant = len(cir_complex) // target_samples
    K_actual = min(K, ue_per_ant)
    # 重塑为（样本数, 用户数, 天线数），天线数固定为1
    cir_2d = cir_complex[:target_samples * ue_per_ant].reshape(target_samples, ue_per_ant, 1)
    print(f"重塑为：（样本数={target_samples}, 用户数={ue_per_ant}, 天线数=1）")

    cir_cropped = cir_2d[:target_samples, :K_actual, :1]  # 截取用户数
...
# 展平信道特征
channel_flat = service_ue_channel.transpose(0, 3, 1, 2).reshape(sample_num_actual, -1)

# 计算RSSI特征
service_power = np.abs(service_ue_channel)** 2
avg_service_power = np.mean(service_power, axis=2)  # （样本数, 基站数, Us）
rssi_matrix = np.zeros((sample_num_actual, N_BS_actual, Us, K_actual), dtype=np.float32)
noise_power = 10 **(-noise_power_dbm / 10)

for i in range(sample_num_actual):
    rssi_matrix[i, :, :, top_us_ue_indices[i]] = avg_service_power[i, :, :, np.newaxis]
    mask = ~np.isin(np.arange(K_actual), top_us_ue_indices[i])
...
dataset = np.load("dataSet4x64x8x4/130dB/dataSet_130.npy")
print("数据集形状：", dataset.shape)  # 应输出 (n_samples, 384)
channel_len = 1*64*4  # 256
rssi_len = 1*4*32     # 128
print("信道长度：", dataset.shape[1] - rssi_len)  # 应输出256
print("RSSI长度：", rssi_len)                    # 应输出128